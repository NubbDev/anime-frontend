<script lang="ts">
    import FilledHouse from '$lib/assets/icons/house-blank-filled.svg?src';
...
<nav>
    ...
    {#each navOptions as option}
        <a href={option.href}>
            {#if path === option.href} 
                    <svelte: this={option.selected} name={option.name} />
                    <span>{option.name}</span>
                {:else}
                    <svelte:component this={option.unSelected} fill="white" name={option.name}/>
